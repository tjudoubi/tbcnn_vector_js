...
def init_net_finetune(feature_size, embeddingg):
    ...
    with tf.name_scope('network'):
        conv = conv_layer(1,600, nodes1, children1, nodes2, children2, feature_size)
        print(conv)
        # h=hidden_layer(pooling,600,50)

    return nodes11, children1,nodes22,children2,conv
...
def pooling_layer(nodes):
    ...
    with tf.name_scope("pooling"):
        pooled1 = tf.reduce_max(nodes[0], axis=1)
        pooled2 = tf.reduce_max(nodes[1], axis=1)
        return pooled1,pooled2
def computCos(pool):
    pool1_norm = tf.sqrt(tf.reduce_sum(tf.square(pool[0]), axis=1))
    pool2_norm = tf.sqrt(tf.reduce_sum(tf.square(pool[1]), axis=1))
    pool1_pool2 = tf.reduce_sum(tf.multiply(pool[0], pool[1]), axis=1)
...
def hidden_layer(pooled,input_size, output_size):
    ...
    with tf.name_scope("hidden"):
        ...
        return tf.nn.tanh(tf.matmul(pooled[0], weights) + biases),tf.nn.tanh(tf.matmul(pooled[1], weights) + biases)

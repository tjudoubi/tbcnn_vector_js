...
def train_model(infile, embeddings):
    ...
    num_feats = 100
    nodes_node1, children_node1, res = network5.init_net_finetune(num_feats,embeddingg)
    aaa = 1
    aaaa = 1
    bbbb = 1
    ...
    labels_node, loss_node, acc = network5.loss_layer(logits_evel)
    ...
    dictt = {}
    listrec = []
    f = open("D:\\tbcnn_train_vector\\test_getSentenceJS\\flist_JSC.txt", 'r')
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        dev_acc_array = []
        f = open('D:/tbcnn_train_vector/test_getSentenceJS/JSC_test/devdata.txt', 'r')
        line = "123"
        k = 0
        dev_batch_q = []
        while line:
            ...
            dev_acc += accur
            dev_batch += 1
                # saver.save(sess, "./model_o/model"+str(aaaa)+".ckpt")
        ...
        dev_acc_array = []
        f = open('D:/tbcnn_train_vector/test_getSentenceJS/JSC_test/testdata.txt', 'r')
        line = "123"
        k = 0
        test_batch_q = []
        while line:
...
if __name__ == '__main__':
    ...
    for i in range(len(listchar)):
        dictta[listchar[i]] = i
    train_model('D:/tbcnn_train_vector/test_getSentenceJS/JSC_test/traindata.txt', dictta)
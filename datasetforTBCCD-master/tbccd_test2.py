...
def train_model(infile, embeddings):
    ...
    num_feats = 100
    nodes_node1, children_node1, nodes_node2, children_node2, conv_ = network_test2.init_net_finetune(num_feats,embeddingg)
    aaa = 1
    pool_=network_test2.pooling_layer(conv_)
    h_=network_test2.hidden_layer(pool_,600,50)
    res=network_test2.computCos(h_)
    labels_node, loss_node = network_test2.loss_layer(res)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        while line:
            ...
            # print(batch_labels)
            _, err, r , h , pool , conv = sess.run(
                [train_step, loss_node, res, h_ , pool_ , conv_],
                feed_dict={
                    nodes_node1: nodes11,
        ...
        while line:
            ...
            output, out_h , out_pool , out_conv = sess.run([res, h_ , pool_ , conv_],
                              feed_dict={
                                  nodes_node1: nodes11,
                                  children_node1: children1,
                                  nodes_node2: nodes22,

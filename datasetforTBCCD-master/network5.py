...
def init_net_finetune(feature_size, embeddingg):
    ...
    with tf.name_scope('inputs'):
        nodes11 = tf.placeholder(tf.int32, shape=(None, None,), name='tree')
...
def conv_layer(num_conv, output_size, nodes1, children1,feature_size):
    ...
    with tf.name_scope('conv_layer'):
        nodes11=conv_node(nodes1, children1,feature_size, output_size)
        nodes1 = [
            nodes11
        ...
        #     for _ in range(num_conv)
        # ]
        return tf.concat(nodes1, axis=2)

def conv_node(nodes1, children1,feature_size, output_size):
    ...
    with tf.name_scope('conv_node'):
        ...
        return conv_step(nodes1, children1, feature_size, w_t, w_r, w_l, b_conv)
...
def conv_step(nodes, children, feature_size, w_t, w_r, w_l, b_conv):
    ...
    with tf.name_scope('conv_step'):
        ...
        with tf.name_scope('combine'):
            ...
            # print("result4:", result.shape)
            result = tf.tensordot(result, weights, [[2, 3], [0, 1]])
            # print("result5:", result.shape)
            # print("weight:",weights.shape)
            # print("bconv:",b_conv.shape)
            # output is (batch_size, max_tree_size, output_size)
            # print(tf.nn.tanh(result + b_conv, name='conv').shape)
...
#     return cosin
def hidden_layer(pooled1,input_size, output_size):
...
def loss_layer(logits_node):
    ...
    with tf.name_scope('loss_layer'):
        # correct_prediction = tf.equal(logits_node, labels)
        # accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_node,labels=labels))

        correct_prediction = tf.equal(tf.cast(tf.argmax(logits_node,1),tf.int32), labels)
        accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
...
def out_layer(logits_node):
    ...
    with tf.name_scope('output'):
        dense1 = tf.layers.dense(inputs=logits_node,
                        units=2,
...
# def acc(logits_node):
#     labels = tf.placeholder(tf.float32, (50,))
#     correct_prediction = tf.equal(logits_node, labels)
#     accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
